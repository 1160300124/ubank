...
(function($){
     ...
     * 编辑全部
     */
    BootstrapTable.prototype.editAll = function(){
        var that = this;
        if (!that.editing) {
            $(this.$body[0].children).each(function(index, item){
                $(item.children).each(function(tdIndex,td) {
     ...
     * 获取编辑状态
     */
    BootstrapTable.prototype.getEditStatus = function(){
        return this.editing;
    }
    /**
     * 编辑全部
     */
    BootstrapTable.prototype.cancelEditAll = function(){
        recoverAll(this);
    }
    ...
    BootstrapTable.prototype.initEdit = function(){
        ...
        $(document).bind('mousedown',function(event){
            ...
            if(!($target.parents().addBack().is(that.$body)) && !($target.parents().addBack().is($('.datetimepicker')))){
                setTimeout(function () {
                    recover(that);
...
//                if(typeof $(this).find('input[type="text"]').bootstrapSelect('getText') != 'object'){
                ...
                if(textVal != undefined){
                    ...
                    if(that.columns[i].edit.required == true){
     ...
     * 恢复表格，使之处于不可编辑状态
     */
    function recoverAll(that){
        $(that.$body[0].children).each(function(index, item){
            var isModi = false;//判断行值是否变动过
            $(item).find('td').closest('td').siblings().html(function(i,html){
                $(this).attr('style',$(this).data('style'));
                var textVal = $(this).find('input[type="text"]').val() || $(this).find('select').val();
                var hiddenVal = $(this).find('input[type="hidden"]').val();
                if(textVal != undefined){
                    if($(this).data('oldVal') != (hiddenVal?hiddenVal:$.trim(textVal)) && $(this).data('field')) {
                        that.data[$(item).data('index')][$(this).data('field')] = hiddenVal?hiddenVal:$.trim(textVal);
                        isModi = true;
                    }
                    if(that.columns[i].edit.required == true){
                        if(textVal == null || textVal == ''){
            ...
            //新值跟旧值不匹配证明被改过
            if(isModi || $(item).hasClass('editable-insert')){
                $(item).addClass('editable-modify');
            }
            else{
                $(item).removeClass('editable-modify');
            }
        });
        that.editing = false;
    }
