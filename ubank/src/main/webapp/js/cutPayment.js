$(function(){
	...
	$("#btn_delete").unbind().bind("click", function(){
		...
		//取表格的选中行数据
		var arrselections = $("#getCutPaymentList").bootstrapTable('getSelections');
	...
	$("#btn_search").unbind().bind("click", function(){
		...
		params.dept = dept;
		params.userName = $("#user_name").val();
		params.cutType = $("#type_select").val();
		params.startDate = $("#start_date").val();
		params.endDate = $("#end_date").val();
		...
		$("#tb_cut_payments").bootstrapTable("refresh", {
			url : "getCutPaymentList?" + parseParams(params)
		});
...
function typeFormatter(value, row, index) {
	if (value == 0 || row.clockOnStatus == 0){
		...
		return "早退";
	} else if (value == 2 || row.clockOnStatus == 2){
		return "未打卡";
	} else if (value == 3 || row.clockOnStatus == 3){
		return "旷工";
	} else if (value == 4 || row.clockOnStatus == 4){
		return "请假";
	}
	return "-";
}
